...
##' @export
setReadable <- function(x, OrgDb, keytype = "auto", keyType="auto") {
    if (!missing(keytype) && !is.null(keytype)) {
        warning("`keytype` is deprecated. Please use `keyType` instead", call.=FALSE)
        keyType = keytype
    }
